package maharjan.diken.musicplayer.ui.custom;
...
import maharjan.diken.musicplayer.R;

public class MusicCoverView extends AppCompatImageView implements Animatable {
    ...
    int getTransitionAlphaValue() {
        return mTrackPaint.getAlpha() * ALPHA_OPAQUE / mTrackAlpha;
    }

    void setTransitionAlphaValue(@IntRange(from = ALPHA_TRANSPARENT, to = ALPHA_OPAQUE) int alpha) {
        if (alpha != getTransitionAlphaValue()) {
            mTrackPaint.setAlpha(alpha * mTrackAlpha / ALPHA_OPAQUE);
            invalidate();
        }
    }
